...
// ── GET /api/papers ── (public, with filters)
router.get('/', async (req, res) => {
  ...
  if (search) {
    conditions.push(`(p.course_code ILIKE $${i} OR p.course_name ILIKE $${i} OR p.department ILIKE $${i})`);
  ...
  try {
    const countResult = await pool.query(
      `SELECT COUNT(*) FROM papers p ${where}`, params
    );
...
// ── PUT /api/papers/:id/approve ── (admin only)
...
// ── PUT /api/papers/:id/reject ── (admin only)
router.put('/:id/reject', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    console.error(err);
    res.status(500).json({ error: 'Failed to reject paper' });
  }
});

// ── DELETE /api/papers/:id ── (admin or uploader)
